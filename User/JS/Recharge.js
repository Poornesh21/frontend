...
const stripePayment = {
    ...
    async loadStripe() {
        return new Promise((resolve, reject) => {
            if (window.Stripe) {
                // Directly resolve with Stripe if already loaded
                ...
                script.onload = () => {
                    // Fetch public key after script loads
                    fetch('/api/stripe/public-key')
                        .then(response => response.json())
                        .then(data => {
                            const stripe = Stripe(data.publicKey);
    ...
    async initiateCheckout(planDetails) {
        try {
            ...
            const checkoutData = {
                ...
                planName: planDetails.name || 'Mobile Recharge Plan'
            };
...
// Global checkout handler for inline use in HTML
window.handleStripeCheckout = function(plan) {
    stripePayment.initiateCheckout(plan);
};
